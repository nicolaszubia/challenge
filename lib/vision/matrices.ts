 ...
 *
 * Matrices operate on linear sRGB. Severity 0 is identity (normal vision).
 * Severity 1.0 is dichromacy (protanopia / deuteranopia / tritanopia).
 *
 * Intermediate intensity uses linear interpolation between adjacent
 * precomputed matrices. Spectra labels this as "simulation intensity"
 * rather than clinical severity: the Machado parameter is a model
 * quantity, not a diagnosis.
 */
...
export function lerpMatrix(a: Matrix3, b: Matrix3, t: number): Matrix3 {
  const mix = (i: number, j: number) => a[i][j] * (1 - t) + b[i][j] * t;
  return [
...
export function matrixForSeverity(tables: Matrix3[], severity: number): Matrix3 {
  const clamped = Math.min(1, Math.max(0, severity));
  const scaled = clamped * 10;
  if (scaled === 0) return IDENTITY_MATRIX;
  if (scaled >= 10) return tables[9];

  if (scaled < 1) {
    return lerpMatrix(IDENTITY_MATRIX, tables[0], scaled);
  }

  const index = Math.floor(scaled);
  const frac = scaled - index;
  if (frac === 0) return tables[index - 1];
  return lerpMatrix(tables[index - 1], tables[index], frac);
}

export function getProtanopiaMatrix(intensity: number): Matrix3 {
  return matrixForSeverity(PROTAN_MATRICES, intensity);
}

export function getDeuteranopiaMatrix(intensity: number): Matrix3 {
  return matrixForSeverity(DEUTAN_MATRICES, intensity);
}

export function getTritanopiaMatrix(intensity: number): Matrix3 {
  return matrixForSeverity(TRITAN_MATRICES, intensity);
}

export function applyMatrix(matrix: Matrix3, r: number, g: number, b: number): [number, number, number] {
  return [
